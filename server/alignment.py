...
def align_benchmark_iter(path, _benchmark, iteration, warm_up=0, bucket=BUCKET_SIZE_MS):
    aligned = []
    for benchmark in [_benchmark]: #Awkward, but I don't want to rock the boat here
        bench = benchmark
        data_dir = os.path.join(path, benchmark)
        try:
            energy = pd.read_csv(os.path.join(data_dir, f'energy_{iteration}.csv'))
            iteration_times = energy.groupby(
                'iteration').timestamp.agg(('min', 'max'))
            probes = []
            for chunk in pd.read_csv(os.path.join(data_dir, f'probes_{iteration}.csv'), chunksize=10**6):
                mask = None
            ...
            ts_first = probes.reset_index().ts.min() * 10**6 * bucket
            ts_last = probes.reset_index().ts.max() * 10**6 * bucket
            if probes.sum() > 0:
                ...
                #     s, norm_with_buckets(bucket)).reset_index('iteration', drop=True))
                power = samples_to_power(df, norm_with_buckets(bucket))
                # print(power)
                ...
                df_energy = pd.read_csv(
                    os.path.join(data_dir, f'energy_{iteration}.csv'))
                df_energy = df_energy[df_energy.iteration > warm_up]
                ...
                for component in components:
                    ...
                    #     s, component, norm_with_buckets(bucket)).reset_index('iteration', drop=True))
                    comp_power = samples_to_power_single_component(df_energy, component, norm_with_buckets(bucket))
                    df = pd.merge(
                        df,
...
def remove_tokens(probes):
    return sorted(set([p[:p.rfind("__")] if "begin" in p or "end" in p or "entry" in p or "return" in p else p for p in probes]))
    
def main():
    print(align_benchmark_iter("/home/jraskin3/testing_GRPC/lucretius/my-fibonacci-original/", "my-fibonacci", 1));
    return
