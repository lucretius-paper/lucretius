 ...
 * {@Samples}.
 */
public final class RaplSampleCollector implements SampleCollector{
  ...
  public void write_iter() {
	...
	int componentCount = iter_samples.get(0).energy[0].length * Rapl.getInstance().getSocketCount();
	writer.println(
		       String.join(
				   ",",
				   "iteration",
  ...
  // TODO: this is a poor practice; we should use something like a builder.
  ...
  private static double[][] getEnergy() {
    return Rapl.getInstance(
            String.join("/", System.getProperty("lucretius.library.path"), "libRapl.so"))
        .getEnergyStats();
  }
